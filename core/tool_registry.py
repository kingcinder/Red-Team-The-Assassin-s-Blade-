...
class ToolRegistry:
    ...
    def _build_command(self, tool: ToolDefinition, args: dict) -> list:
        ...
        if name in _nmap:  return self._build_nmap(name, args, binary)
        if name == "masscan_scan":     return self._build_masscan(args, binary)
        if name == "nikto_scan":       return self._build_nikto(args, binary)
        if name == "sqlmap_scan":      return self._build_sqlmap(args, binary)
        if name == "gobuster_dir":     return self._build_gobuster(args, binary)
        if name == "hydra_brute":      return self._build_hydra(args, binary)
        if name == "john_crack":       return self._build_john(args, binary)
        if name == "hashcat_crack":    return self._build_hashcat(args, binary)
        ...
        if name == "tcpdump_capture":  return self._build_tcpdump(args, binary)
        if name == "socat_relay":      return self._build_socat(args, binary)
        ...
        if name in ("whois_lookup", "waf_detect", "exiftool_read", "exiftool_osint"):
            return self._simple_positional(binary, args, ["target", "file"])
        if name in ("searchsploit_search", "searchsploit_exploit"):
            return self._simple_positional(binary, args, ["query"])
        if name in ("enum4linux_enum", "nbtscan_scan", "smbmap_enum", "snmpwalk_enum", "onesixtyone_scan"):
            return self._simple_positional(binary, args, ["target"])
        if name in ("theharvester_gather", "amass_enum", "subfinder_enum", "dnsx_probe",
            ...
            return self._simple_positional(binary, args, ["domain","url","target"])
        if name == "whatweb_scan":
            return self._build_whatweb(args, binary)
        if name == "dns_enum":
            return self._build_dnsenum(args, binary)
        if name == "wget_download":
            return self._build_wget(args, binary)
        if name == "dig_dns":
            return self._build_dig(args, binary)
        if name == "snmpwalk_enum":
            return self._build_snmpwalk(args, binary)
        if name == "crunch_gen":
            return self._build_crunch(args, binary)
        if name == "binwalk_analyze":
            return self._build_binwalk(args, binary)
        if name == "foremost_carve":
            return self._build_foremost(args, binary)
        if name == "strings_extract":
            return self._build_strings(args, binary)
        if name == "cewl_gen":
            return self._build_cewl(args, binary)
        if name == "httpx_probe":
            return self._build_httpx(args, binary)
        if name == "objdump_disasm":
            return self._build_objdump(args, binary)
        if name == "readelf_analyze":
            return self._build_readelf(args, binary)
        if name in ("strace_trace", "ltrace_trace"):
            return self._build_trace(name, args, binary)
        if name == "gdb_debug":
            return self._build_gdb(args, binary)
        if name == "apktool_decompile":
            return self._build_apktool(args, binary)
    ...
    def _build_nmap(self, name, args, binary):
        cmd = [binary]
        ...
        if name == "nmap_vuln_scan":
            ...
            if st: cmd.append(st)
            cmd.extend(["-oN", f"{self._output_dir}/nmap_{target.replace('/','_').replace('.','_')}.txt"])
    ...
    def _build_sqlmap(self, args, binary):
        ...
        cmd.extend(["--output-dir", self._output_dir])
        return cmd

    def _build_gobuster(self, args, binary):
    ...
    def _build_hashcat(self, args, binary):
    ...
    def _build_curl(self, args, binary):
        cmd = [binary, "-s", "-i"]
    ...
    def _build_msfvenom(self, args, binary):
    ...
    def _build_socat(self, args, binary):
        cmd = [binary]
    ...
    def _simple_positional(self, binary, args, key_order):
